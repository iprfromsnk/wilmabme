 ...
 ******************************************************************************/
#warning This interface is not supported. Use nwkAttributes.h, nwkNeighbor.h and nwkSecurity.h
...
/******************************************************************************
                                 Types section
 ******************************************************************************/
/* Identificators of network information base attributes. ZigBee spec r17, 
 * Table 3.44. */
typedef enum _NWK_NibId_t
{
   ...
   * all child devices to retransmit a broadcast message. Valid range: 0-0x2710. */
  ...
  /* If this is set to 0, the NWK layer shall calculate link cost from all neighbor
   * nodes using the LQI values reported by the MAC layer; otherwise, it shall
   ...
   * outgoing packets. '1' - time indication provided. '0' no time indication provided. */
  NWK_NIB_TIME_STAMP_ID             = 0x8C,
  /* The current route symmetry setting. 'true' means that routes are considered to be
   * comprised of symmetric links. Backward and forward routes are created during one-route 
   * discovery and they are identical. 'false' indicates that routes are not consider to
   * be comprised of symmetric links. Only the forward route is stored during route discovery.*/
  NWK_NIB_SYM_LINK_ID               = 0x8E,
  /* This field shall contain the device capability information established at network 
   * joining time. */
  ...
  /* A flag that determines whether the NWK layer should assume the ability to use 
  ...
  /* Set of network security material descriptors capable of maintaining an active and alternate
   * network key.*/
  ...
  /* Indicates whether incoming NWK frames must be all checked for freshness when the
   * memory for incoming frame counts is exceeded. */
  NWK_NIB_ALL_FRESH_ID              = 0xA3,
  /* Indicates whether security shall be applied to incoming and outgoing NWK data frames. 
   * If set to 0x01, security processing shall be applied to all incoming and outgoing frames except data
   * frames destined for the current device that have the security sub-field of the frame control field
   * set to 0. If this attribute has a value of 0x01, the NWK layer shall not relay frames that have 
   * the security sub-field of the frame control field set to 0. The SecurityEnable parameter of 
   * the NLDEDATA.request primitive shall override the setting of this attribute. */
...
/**************************************************************************//**
  ...
  \param req - NLME-SET request parameters' structure pointer.
  \return none.
 ******************************************************************************/
...
/**************************************************************************//**
  ...
  \param req - NLME-GET request parameters' structure pointer.
  \return none.
 ******************************************************************************/
