...
{
  Timer_t *p, *prev; // p is bottom of list
  uint32_t sysTime;
  uint32_t interval;

  // start timer queue algorithm
  prev = p = halAppTimerHead;
  for (; p ;)
  {
    sysTime = halGetTimeOfAppTimer();
    interval = sysTime - p->service.sysTimeLabel;
    if (interval >= p->interval)
    {// interval was completed
      if (TIMER_ONE_SHOT_MODE == p->mode)
      {
        halRemoveTimer((Timer_t**)(&halAppTimerHead), prev, p);
      }
      else
      {
      ...
      p->callback();
      /* We must start passing through top of the list. Because user can delete some
         elements of the list by HAL_StopAppTimer within callback.
      */
      p = halAppTimerHead;
      continue;
    }
...
int HAL_StartAppTimer(HAL_AppTimer_t *appTimer)
{
  ...
  halAddTimer((Timer_t**)(&halAppTimerHead), (Timer_t*)appTimer);
  return 0;
}
