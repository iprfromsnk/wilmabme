...
{
  /* Route discovery and/or repair has been attempted and no route to the intended 
   * destination address has been discovered. */
  NWK_NO_ROUTE_AVAILABLE            =  0x00,
  /* The routing failure occurred as a result of the failure of an attempt to route 
   * the frame along the tree. */
  ...
  /* The failure occurred as the result of an attempt to buffer a frame for a sleeping 
   * end device child and the relaying device had no buffer capacity to use. */
  NWK_NO_INDIRECT_CAPACITY          =  0x05,
  /* A frame that was buffered on behalf of a sleeping end device child has been dropped 
   ...
   * This status is only used locally on a device to indicate loss of communication with 
   * the parent. */
  NWK_PARENT_LINK_FAILURE           =  0x09,
  /* The multicast route identified in the destination address field should be validated. */
  NWK_VALIDATE_ROUTE                =  0x0A,
  /* Source routing has failed, probably indicating a link failure in one of the source 
   * route’s links. */
  ...
  /* The address in the destination address field has been determined to be in use by 
   * two or more devices. */
  NWK_ADDRESS_CONFLICT              =  0x0D,
  /* The source device has the IEEE address in the Source IEEE address field and, 
   * if the Destination IEEE address field is present, the value it contains is the expected 
   * IEEE address of the destination. */
  ...
  /* A frame counter reported in a received frame had a value less than or equal to 
   * that stored in nwkSecurityMaterialSet. */
...
/**//**
 * \brief NLME-NWK-STATUS indication primitive's parameters structure declaration.
