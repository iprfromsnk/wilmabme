 ...
 ******************************************************************************/
/**//**
 * \brief NLME-ROUTE-DISCOVERY confirm primitive's parameters structure declaration.
...
typedef struct _NWK_RouteDiscoveryConf_t
{
  ...
  NWK_Status_t  status;
  /* In the case where the Status parameter has a value of ROUTE_ERROR, this code gives 
   * further information about the kind of error that occurred. Otherwise, it should be
   * ignored. ZigBee spec r17, Table 3.35.
   * */
  NWK_StatusIndErrorCodes_t  networkStatusCode;
} NWK_RouteDiscoveryConf_t;

/**//**
 * \brief NLME-ROUTE-DISCOVERY request primitive's parameters structure declaration.
...
typedef struct _NWK_RouteDiscoveryReq_t
{
  ...
  ShortAddr_t  dstAddr;
  /* This optional parameter describes the number of hops that the route request 
   * will travel through the network. */
  uint8_t  radius;
  /* In the case where DstAddrMode has a value of zero, indicating many-to-one route
   * discovery, this flag determines whether the NWK should establish a route record table.
   * TRUE = no route record table should be established, FALSE = establish a route record table */
  bool  noRouteCache;
...
/**************************************************************************//**
  ...
  \param req - NLME-ROUTE-DISCOVERY request parameters' structure pointer.
  \return none.
 ******************************************************************************/
